...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Bravo {user.username}..We Welcome You To Our Website😍', 'succes')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash(
                f'Bravo {user.username}..Welcome Back😍', 'succes')
            next_page = request.args.get('next')
            # user = User.query.all()
    ...
    return render_template('login.html', form=form, login_register='Login')
